...
namespace CoreClass
{
	...
	/// T-Sql语句 存储过程执行类
	/// </summary>
	public class OperateSqlClass_SQLITE : SqlProc_SQLITE
	{
		/// <summary>
		/// 打开数据库连接
		/// </summary>
        ...
        public bool Insert(string sql, ref long identity)
        {
            bool b = false;
            m_sql = sql;
            sql += ";select CAST(LAST_INSERT_ROWID() as bigint)";
		...
		private string[] getIdentityCells(string tablename)
		{
			...
			if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
			{
				DataRow[] drArr = ds.Tables[0].Select("pk=1");
				int k = drArr.Length;
				string[] arr = new string[k];
				for (int i = 0; i < k; i++)
				{
					arr[i] = drArr[i]["name"].ToString();
				}
				return arr;
			}
			return null;
