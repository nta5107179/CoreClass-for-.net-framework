...
namespace CoreClass
{
	...
	/// T-Sql语句 存储过程执行类
	/// </summary>
	public class OperateSqlClass_MYSQL : SqlProc_MYSQL
	{
		/// <summary>
		/// 打开数据库连接
		/// </summary>
		...
		{
            m_connectionstring = ConfigurationManager.ConnectionStrings[connectionstring].ToString();
            MySqlProcOpen();
		}
		/// <summary>
		/// 关闭数据库连接
        ...
        public bool Insert(string sql, ref long identity)
        {
            bool b = false;
            m_sql = sql;
            sql += ";select CAST(LAST_INSERT_ID() as SIGNED) as identity";
			...
			try
			{
				for (int i = 0; i < objarr.Length; i++)
				{
					...
					if (strarr != null)
					{
						...
						string sql = string.Format("delete from {0} where 1=1{1}", obj.GetType().Name, str);
		...
		{
			DataSet ds = new DataSet();
            string sql = string.Format("show columns from {0} where extra='auto_increment'", tablename);
			m_sql = sql;
			Execute(ds);
			if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
			{
				...
				for (int i = 0; i < k; i++)
				{
					arr[i] = ds.Tables[0].Rows[i]["field"].ToString();
				}
				return arr;
			}
			return null;
